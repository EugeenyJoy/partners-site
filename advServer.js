   ...
   app.get('/ipList', (req, res2) => {
    ...
    app.get('/refItems_delOld', (req, res2) => {
        ...
        pool.query("SELECT id FROM refItems WHERE isAdmin = 0 and NOW() > DATE_ADD(date_added, INTERVAL 2 DAY);", (error, res3) => {
            ...
            // Удаляет записи старше 2 дней
            pool.query("DELETE FROM refItems WHERE isAdmin = 0 and NOW() > DATE_ADD(date_added, INTERVAL 2 DAY)", (error, result) => {
                if (error) throw error;
    ...
    var requestLoop = setInterval(function(){
        http.get(`http://advertemp.com/refItems_delOld`);
      }, 60000);
...
const httpsServer = https.createServer({
  key: fs.readFileSync('/home/admin/conf/web/advertemp.com/ssl/advertemp.com.key'),
  cert: fs.readFileSync('/home/admin/conf/web/advertemp.com/ssl/advertemp.com.pem'),
}, app);
...
// Запуск задачи очистки каждый час
cron.schedule('0 * * * *', () => {
